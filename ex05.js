...
If we want to add a parameter to a url, there are certain characters that must be encoded 
in order to make the URL valid. There are many characters that must be encoded, 
including: , !, ", and #. For this exercise, we will only be focusing on replacing the space with %20.
...
forEach
define the variable outside the forEach method, and same as return you can't return inside of forEach method
*/

const urlEncode = function (text) {
  let joinedText = {};
  const splitText = text.trim().split(" ")
  joinedText = splitText.join("%20")
  return joinedText;
};
...
// if (text[0] === null || text[text.length] === null){
  //   text.trim();
  // } else {
  //   const splitText = text.split(" ")
  //   joinedText = splitText.trim().join("%20")
  // }
  

  // 文字列の中の空白を調べたい
  // 空白があったらそれを%20に置き換える、でも文字列の外側の空白は置き換えない
  // splitでwordごとに文字を分けて変数に入れる
  // 分けた文字に%20をつけてjoin
  // ただ、最初と最後のインデックスに空白がある場合は文字を分けないで無視する